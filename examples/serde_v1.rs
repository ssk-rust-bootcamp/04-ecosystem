...
/// Deserialize base64 encoded string to Vec<u8>
fn b64_decode<'de, D>(deserializer: D) -> Result<Vec<u8>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let encode = String::deserialize(deserializer)?;
    let decode = URL_SAFE_NO_PAD.decode(encode.as_bytes()).map_err(serde::de::Error::custom)?;
    Ok(decode)
}
#[allow(dead_code)]
